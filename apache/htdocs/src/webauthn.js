...
function checkUserExists() {
    ...
    $.get('auth2/userexists?uname=' + state.user.name, {}, null, 'json')
...
function getCredentials() {
    ...
    $.get('auth2/credential/' + state.user.name, {}, null, 'json')
        .done(function (response) {
            console.log(response)
        });
}
...
function makeCredential() {
    ...
    //$.get('auth/FidoWebAuth.exe/enroll?uname=' + state.user.name, {  // actually...we only need the uname...
    $.get('auth2/enroll?uname=' + state.user.name, {  // actually...we only need the uname...
...
function DoAssert() {
	...
	$.get('auth2/assertstart?uname=' + state.user.name, {
...
function verifyAssertion(assertedCredential) {
    ...
    $.ajax({
        ...
        dataType: "json",
        success: function (response) {
            showErrorAlert("Response from server: " + response.success);
            console.log(response)
        }
    });
}
